...
if (!isset($_SESSION['dni'])) {
	...
	$page_title = 'Edit the Status of the task';
	echo '<h1>Edit Status Task</h1>';
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if ($_POST['sure'] == 'Yes') { 
			...
			// Make the query:
			$q = "UPDATE tasks SET State='Pending', Time_Finish=NOW() WHERE id=$id LIMIT 1";		
			...
			if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.
				...
				echo '<p>The task has been closing</p>';
				echo 'Wait a moment...'; 
		...
		if (mysqli_num_rows($r) == 1) { // Valid task ID, show the form.
			...
			// Create the form:
			echo '<form action="edit_status_tasks.php" method="post">
